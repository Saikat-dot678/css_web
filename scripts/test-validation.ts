...
for (const value of ["https://example.com/path?q=1", "http://localhost:3000/test", "/uploads/image.png", "/api/posters/demo", "#"]) {
  assert.equal(urlSchema.safeParse(value).success, true, `expected safe URL: ${value}`);
}

for (const value of ["javascript:alert(1)", "data:text/html,test", "file:///etc/passwd", "ftp://example.com/file", "//evil.example/path", " /safe-looking", "/path with space"]) {
...
console.info("URL scheme, protocol-relative path, and resource boundary validation passed.");
