...
export type ScreenTakeoverHandle = {
  setProgress: (progress: number) => void;
  syncSourceRect: () => void;
};

type ScreenTakeoverProps = {
  academicYear: string;
  memberCount: number;
  nextEvent?: string;
...
export const ScreenTakeover = forwardRef<ScreenTakeoverHandle, ScreenTakeoverProps>(function ScreenTakeover(
  { academicYear, memberCount, nextEvent, sourceRef },
  forwardedRef,
) {
  ...
  const setProgress = useCallback((rawProgress: number) => {
    ...
    const left = mix(source.left, 0, eased);
    const top = mix(source.top, 0, eased);
    const right = mix(source.right, 0, eased);
    const bottom = mix(source.bottom, 0, eased);
    aperture.style.clipPath = `inset(${top}px ${right}px ${bottom}px ${left}px round ${mix(source.radius, 0, eased)}px)`;
    root.style.setProperty("--takeover", String(progress));
    root.dataset.active = String(progress > .008);
    root.dataset.complete = String(progress > .985);
  }, [syncSourceRect]);

  useImperativeHandle(forwardedRef, () => ({ setProgress, syncSourceRect }), [setProgress, syncSourceRect]);
  useEffect(() => {
    const onResize = () => syncSourceRect();
  ...
  return (
    <div ref={rootRef} className={styles.takeover} style={{ "--takeover": "0" } as CSSProperties} data-active="false" data-complete="false">
      <div ref={apertureRef} className={styles.aperture}>
        <div className={styles.mapPlane} aria-hidden="true">
          <CampusScreenMap variant="takeover" revealProgress={1} routeProgress={1} focusProgress={1} />
        </div>

        <article className={styles.identity} aria-label="Computer Science Students' Society">
          <div className={styles.registration} aria-hidden="true"><i /><i /><i /><i /></div>
          <div className={styles.topline}>
            <span>DEPARTMENT DISPATCH / ISSUE 04</span>
            <strong>CSS / NITDGP</strong>
          </div>
          ...
          <div className={styles.footer}>
            ...
            <div>
              <Link className="v4-button v4-button-primary" href="#about">Enter CSS ↓</Link>
              <Link className="v4-text-link" href="/events">View events ↗</Link>
            </div>
          </div>
          <aside className={styles.stats} aria-hidden="true">
            <span>{memberCount} / MEMBERS</span>
            <span>{academicYear} / SESSION</span>
