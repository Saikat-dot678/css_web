...
export default async function TeamPage({ searchParams }: { searchParams: Promise<{ year?: string }> }) {
  ...
  return (
    <PublicShell>
      <main id="main" className="v4-page v4-team-page">
        <V4PageHero
          index="05"
          eyebrow={`${selectedYear === content.currentAcademicYear ? "TEAM / CURRENT COHORT" : "TEAM / COMMITTEE ARCHIVE"}`}
          title="The people behind CSS."
          copy="Students across B.Tech, M.Tech and PhD programmes run the society, while faculty coordinators provide continuity beyond a single academic year."
          meta={[`${people.length} MEMBERS`, `${office.length} OFFICE BEARERS`, `${faculty.length} FACULTY COORDINATORS`]}
        >
          <div className="v4-team-collage">
            {people.slice(0, 6).map((member, index) => (
              <div className={`v4-team-face face-${index + 1}`} key={member.id}>
                <Image src={member.photo} alt="" width={320} height={320} unoptimized={member.photo.endsWith(".svg") || member.photo.startsWith("/api/")} />
              </div>
            ))}
            <span>PEOPLE / {selectedYear}</span>
          </div>
        </V4PageHero>
        ...
        {archive && <section className="archive-context wrap reveal"><p className="kicker">Contribution highlights / {archive.academicYear}</p><ul>{archive.contributionHighlights.map((highlight) => <li key={highlight}>{highlight}</li>)}</ul></section>}
        ...
        <section className="team-distribution-new wrap reveal"><div><p className="kicker">Team distribution</p><h2>{people.length} people, clearly organised.</h2></div><div className="distribution-grid"><div><span>Office Bearers</span><strong>{office.length}</strong></div><div><span>Third Year</span><strong>{third.length}</strong></div><div><span>Second Year</span><strong>{second.length}</strong></div><div><span>M.Tech / PhD</span><strong>{mtech.length + phd.length}</strong></div><div className="total"><span>Total</span><strong>{people.length}</strong></div></div></section>
        <section className="cohort-archive wrap"><div><p className="kicker">Previous Committees</p><h2>The people who helped build CSS before us.</h2><p>Every committee remains browsable by year with the same card structure and social links.</p></div><div className="cohort-grid">{committees.map((committee, index) => <article className="reveal" key={committee.id}><span>{committee.academicYear}</span><strong>{committee.memberIds.length}</strong><p>{index === committees.length - 1 ? "Founding committee" : "Previous committee"}</p><Link href={`/team?year=${committee.academicYear}`}>Explore cohort ↗</Link></article>)}</div></section>
        <section className="join-block wrap reveal" id="recruitment"><div><p className="kicker">{content.recruitmentOpen ? "Recruitment open" : "Recruitment"}</p><h2>{content.recruitmentOpen ? "See yourself in the next cohort?" : "Recruitment is currently closed."}</h2><p>{content.recruitmentText}</p></div><div><Link className="solid-link large" href="/#contact">{content.recruitmentOpen ? "Apply to CSS" : "Back to home"}</Link></div></section>
      </main>
    </PublicShell>
  );
}
